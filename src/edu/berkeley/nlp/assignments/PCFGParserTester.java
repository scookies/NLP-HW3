...
public class PCFGParserTester {
  ...
  static class BaselineParser implements Parser {
    ...
    private double bestScoreUnary(int i, int j, String nonTerm, List<String> sentence)
    {
        if(i==j)
        System.out.println("Same i,j - "+ i + " Unary nonTerm - " + nonTerm);

        //return the best score up to this unary
        //baseline
        double bestScore = 0.0;
        if(i==j || i+1 == j)  //looking for binary rule but since there is just one word, there must be a score
        {
            bestScore = lexicon.scoreTagging(sentence.get(i), nonTerm); //if not there return 0?
        }
        if(i!=j)//do below no matter what
        {
            //double score = 0.0;
            for(UnaryRule ur: uc.getClosedUnaryRulesByParent(nonTerm))
            {
                double score = ur.getScore() * bestScoreBinary(i, j, ur.getChild(), sentence);   //think of the unary closure of this case
                if(score>bestScore)
                    bestScore = score;
            }
        }
    ...
    private double bestScoreBinary(int i, int j, String nonTerm, List<String> sentence)
    {

        if(i==j)
            System.out.println("Same i,j - " + i + " Binary nonTerm - " + nonTerm);
        //return the best score up to this binary
        //Base
        double bestScore = 0.0;
        if(i==j || i+1==j)  //looking for binary rule but since there is just one word, there must be a score
        {
        ...
        if(i!=j)
        {
            //for each possible rules for this head
            for(BinaryRule br: grammar.getBinaryRulesByParent(nonTerm))
            {

                //for each s from i to j-1
                for(int s = i; s<=j-1; s++)
                {
                    double score = 0.0;
                    //score calculate by calling unary of left and right - replace it with table lookup for values
                    score = br.getScore();
                    score = score * bestScoreUnary(i,s,br.getLeftChild(), sentence);
                    score = score * bestScoreUnary(s+1, j, br.getRightChild(), sentence);
                    if(score > bestScore)
                        bestScore = score;
                }
    ...
    public Tree<String> getBestParseHelper(List<String> sentence)
    {
        double bestscore = 0;
        //iterate all phase length
        for (int l = 1; l < sentence.size() - 1; l++)
        {
            System.out.println("l = " + l);
            //iterate all phase of length l
            for(int i= 1; i < sentence.size() -l ; i++)
            {
                System.out.println("i = " + i);
                int j = i+l;
                //iterate all rules starting from unary rules
                for(String nonTerminals : grammar.getStates())//UnaryRule ur : grammar.getUnaryRules())
                {
                    double score = bestScoreUnary(i, j, nonTerminals, sentence); //TODO: double check whether this should be child or parents
                    if(score > bestscore)
                        bestscore = score;
                    System.out.println("UnaryScore of " + nonTerminals + "(i,j) - (" + i + ","+ j +  ") : " + score);
                    score = bestScoreBinary(i, j, nonTerminals, sentence);
                    if(score > bestscore)
                        bestscore = score;
                    System.out.println("BinaryScore of " + nonTerminals + "(i,j) - (" + i + ","+ j +  ") : " + score);
                }
  ...
  static class TreeAnnotations {
    ...
    private static Tree<String> binarizeTree(Tree<String> tree) {
      ...
      // otherwise, it's a binary-or-more local tree, so decom`pose it into a sequence of binary and unary trees.
...
//          if (path.size() == 2) continue;
  ...
  public static void main(String[] args) {
    ...
    List<Tree<String>> trainTrees = readTrees(basePath, 200, 200/*2199*/, maxTrainLength);
    ...
    if (testMode.equalsIgnoreCase("validate")) {
      ...
      testTrees = readTrees(basePath, /*2200*/200, /*2299*/200, maxTestLength);
    } else {
      System.out.print("Loading test trees (section 23) ... ");
      testTrees = readTrees(basePath, 200/*2300*/,/* 2399*/200, maxTestLength);
    }
